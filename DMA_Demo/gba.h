...
#define DMA_SOURCE_RESET          (3 << 23) // Reset source to beginning when repeating (apparently a forbidden option)

#define DMA_REPEAT (1 << 25) // Repeat copy every time DMA_AT condition is reached
...
#define DMA_AT_REFRESH (3 << 28) // DMA1 and DMA2, refill FIFO once empty. DMA3, transfer starts at scanline==2 and repeats until scanline==162

#define DMA_IRQ (1 << 30) // Request an interrupt once DMA is finished

#define DMA_ON (1 << 31) // Enable DMA!!! (Very important)
