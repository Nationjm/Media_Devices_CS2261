...
void initialize() {
    ...
    (*(volatile unsigned short*)0x4000008) = (0 << 7) | (0<<14) | ((0) << 2) | ((31) << 8) | 1;

    state = GAME;

    DMANow(3, &tilesTiles, &((charblock *)0x06000000)[0], 64 / 2);
    DMANow(3, &tilesPal, ((unsigned short *)0x5000000), 512 / 2);


    u16 emptyTile = 0;
    DMANow(3, &emptyTile, &((screenblock *)0x6000000)[31], 1024 | (2 << 23));
    for (int i = 0; i < 160 / 8; i++) {
        ((screenblock *)0x6000000)[31].tilemap[((i) * (32) + (0))] = 1;
        ((screenblock *)0x6000000)[31].tilemap[((i) * (32) + (240 / 8 - 1))] = 1;
    }
    for (int i = 0; i < 240 / 8; i++) {
        ((screenblock *)0x6000000)[31].tilemap[((0) * (32) + (i))] = 1;
        ((screenblock *)0x6000000)[31].tilemap[((160 / 8 - 1) * (32) + (i))] = 1;
    }
...
void draw() {
    if (state == GAME) {
        ...
        shadowOAM[shadowOAMIndex].attr2 = (1 + direction) | (((1) & 3) << 10);
        shadowOAMIndex++;
...
# 217 "main.c"
            ...
            shadowOAM[shadowOAMIndex].attr0 = curr->position.y * 8;
            shadowOAM[shadowOAMIndex].attr1 = curr->position.x * 8;
            shadowOAM[shadowOAMIndex].attr2 = 0 | (((1) & 3) << 10);
            shadowOAMIndex++;
        }
        ...
        for (int i = 0; i < 1; i++) {
            ...
            shadowOAM[shadowOAMIndex].attr2 = 5;
            shadowOAMIndex++;
        }
