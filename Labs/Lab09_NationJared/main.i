...
void update() {
    ...
    if ((~(buttons) & ((1<<6))) && colorAt(leftX, topY) && colorAt(rightX, topY)) {
        if (player.y > 0) {
            player.y -= player.yVel;
        }
        player.direction = UP;
        player.isMoving = 1;
    } else if ((~(buttons) & ((1<<7))) && colorAt(leftX, bottomY) && colorAt(rightX, bottomY)) {
    ...
    if ((~(buttons) & ((1<<5))) && colorAt(leftX, topY) && colorAt(leftX, bottomY)) {
        if (player.x > 0) {
            player.x -= player.xVel;
        }
        player.direction = LEFT;
        player.isMoving = 1;
    } else if ((~(buttons) & ((1<<4))) && colorAt(rightX, topY) && colorAt(rightX, bottomY)) {
    ...
    shadowOAM[player.oamIndex].attr0 = (2<<14) | (((player.y - vOff)) & 0xFF);
