...
void initGame() {
    ...
    DMANow(3, connectFourTiledMap, &CHARBLOCK[1], connectFourTiledMapLen / 2);
...
void animateChip() {
    ...
    if (chipSprite.timeUntilNextFrame == 0) {
        chipSprite.timeUntilNextFrame = 10;
        chipSprite.currentFrame = (chipSprite.currentFrame + 1) % chipSprite.numFrames;
    }
    ...
    shadowOAM[chipSprite.oamIndex].attr2 = ATTR2_PALROW(currentColor->spritePaletteRow) | (512 + ATTR2_TILEID(chipSprite.currentFrame * 2, 0));
...
void dropChip() {
    ...
    if (TILEMAP_ENTRY_TILEID(SCREENBLOCK[8].tilemap[OFFSET(9 + currentColumn, 5 + depth, 32)]) != EMPTY_CELL) {
        ...
        if (gameStatus != DRAW) {
            ...
            for (int i = 0; i < 14; i++) {
                if (i < 7) {
                    (SCREENBLOCK[8].tilemap[OFFSET(10 + i, 13, 32)]) |= TILEMAP_ENTRY_PALROW(currentColor->winTextPaletteRow);
                } else {
                    (SCREENBLOCK[8].tilemap[OFFSET(10 + i - 7, 14, 32)]) |= TILEMAP_ENTRY_PALROW(currentColor->winTextPaletteRow);
                }
            }
    ...
    // Make the chip fall one tile every 10 frames
        ...
        if (depth > 0) {
            ...
            SCREENBLOCK[8].tilemap[OFFSET(9 + currentColumn, depth + 4, 32)] = TILEMAP_ENTRY_TILEID(EMPTY_CELL);
